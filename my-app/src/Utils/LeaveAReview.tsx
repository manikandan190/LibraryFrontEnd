...
export const LeaveAReview: React.FC<{ submitReview: any }> = (props) => {
    ...
    const [reviewDescription, setReviewDescritpion] = useState('');
    ...
    return (
        <div className="dropdown" style={{ cursor: 'pointer' }}>
            ...
            {
                displayInput && <form method="POST" action='#'>
                    <hr />
                    <div className="mb-3">
                        <label className="form-label">Description</label>
                        <textarea className="form-control" id="submitReviewDescription" placeholder="Optional" rows={3} onChange={e => setReviewDescritpion(e.target.value)}>
                        </textarea></div>
                    <div>
                        <button type="button" onClick={() => props.submitReview(starInput, reviewDescription)} className="btn btn-primary mt-3">Submit Review</button></div> </form>
            }
        </div>
    );
}